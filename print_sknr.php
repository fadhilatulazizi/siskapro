...
.page{
    ...
    position:relative;
    overflow:hidden;
    page-break-after: always; /* Membuat halaman baru otomatis setiap ganti siswa */
}
/* ================= WATERMARK ================= */
.watermark{
    position:absolute;
...
.watermark-logo img{ width:480px; height:600px; }
...
foreach ($ids as $id): 
    ...
    // Query data siswa menggunakan prepared statement bawaan sytem cetak massal sebelumnya
    ...
    // Cek pembatasan hak akses kelas
...
"); 
    ...
    // Reset variabel perhitungan ke 0 setiap ganti siswa baru
    $no = 1;
    $total_s1 = 0;
    $total_s2 = 0;
    $total_s3 = 0;
...
<div class="page">
    ...
    <div class="title">
        ...
        <p>Nomor : 400.3.11.1 / 150.<?= htmlspecialchars($siswa['no_surat']) ?> / 2026</p>
    </div>
    ...
    <table class="nilai-table">
        <thead>
            <tr>
                ...
                <th rowspan="2" class="nilai">Nilai rata-rata nilai semester I - V</th>
            </tr>
            <tr>
        ...
        while($n = $nilai->fetch_assoc()):
            $rata = ($n['s1'] + $n['s2'] + $n['s3'] + $n['s4'] + $n['s5']) / 5;
...
echo $mapel_tampil;
?></td>
                <td class="nilai"><?= number_format($n['s1'],2,',','.') ?></td>
                <td class="nilai"><?= number_format($n['s2'],2,',','.') ?></td>
                <td class="nilai"><?= number_format($n['s3'],2,',','.') ?></td>
                <td class="nilai"><?= number_format($n['s4'],2,',','.') ?></td>
                <td class="nilai"><?= number_format($n['s5'],2,',','.') ?></td>
                <td class="nilai" style="font-weight:bold;"><?= number_format($rata,2,',','.') ?></td>
            </tr>
        <?php
        $no++;
        endwhile;
        
        // Pengaman division by zero
        $pembagi = max($jumlah, 1);
        ?>
            <tr>
                <td colspan="2" style="text-align:center;font-weight:bold;">Rerata Total</td>
                <td class="nilai" style="font-weight:bold;"><?= number_format($total_s1 / $pembagi,2,',','.') ?></td>
                <td class="nilai" style="font-weight:bold;"><?= number_format($total_s2 / $pembagi,2,',','.') ?></td>
                <td class="nilai" style="font-weight:bold;"><?= number_format($total_s3 / $pembagi,2,',','.') ?></td>
                <td class="nilai" style="font-weight:bold;"><?= number_format($total_s4 / $pembagi,2,',','.') ?></td>
                <td class="nilai" style="font-weight:bold;"><?= number_format($total_s5 / $pembagi,2,',','.') ?></td>
                <td class="nilai" style="font-weight:bold;"><?= number_format($total_rata / $pembagi,2,',','.') ?></td>
            </tr>
        </tbody>
    </table>
